/* eslint-disable no-useless-escape, no-self-compare */
...
function App() {
  ...
  const handlePlantationSaved = (payload) => {
    const plantationPayload = payload?.plantation || {};
    const plantationId =
      payload?.plantationId ||
      plantationPayload?.id ||
      plantationPayload?._id ||
      payload?.health?.plantation?.id ||
      payload?.health?.plantation?._id;

    if (!plantationId) {
      console.error("Invalid plantation save payload:", payload);
      alert("Plantation was saved incorrectly. No plantation ID was returned.");
      return;
    }

    const normalizedPlantation = {
      ...plantationPayload,
      id: String(plantationId),
      _id: String(plantationId)
    };

    setPlantation(normalizedPlantation);
    setHealth(payload?.health || null);

    localStorage.setItem("ndviData", JSON.stringify({
      plantationId,
      plantationName: normalizedPlantation.name,
      location: normalizedPlantation.locationName,
      email: normalizedPlantation.email,
      lat: normalizedPlantation.latitude,
      lon: normalizedPlantation.longitude
    }));
  };
  ...
  if (showAnalysisPage) {
    return <CropAnalysis onBack={() => setShowAnalysisPage(false)} />;
  }
  return (
    <>
      <style>{styles}</style>
      <div id="app-root">
        ...
        <main className="content-wrapper" id="content-wrapper">
          ...
          <section className="map-section" id="map-section">
            ...
            <MapSelector onPolygonSelect={handlePolygonSelect} onPlantationSaved={handlePlantationSaved} />
          </section>
...
{health && (
  ...
  <button
    style={{
      ...
      borderRadius: "8px",
      border: "none",
      background: plantation?.id ? "#4CAF50" : "#7a7a7a",
      color: "white",
      cursor: plantation?.id ? "pointer" : "not-allowed"
    }}
    disabled={!plantation?.id}
    onClick={() => {
      localStorage.setItem("ndviData", JSON.stringify({
        plantationId: plantation?.id,
        plantationName: plantation?.name,
        location: plantation?.locationName || "Selected Field",
        email: plantation?.email,
        lat: plantation?.latitude || analysis.coordinates?.[0]?.lat,
        lon: plantation?.longitude || analysis.coordinates?.[0]?.lng
      }));
